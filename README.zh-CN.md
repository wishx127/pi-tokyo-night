...
**Neon Studio** —— 使用 `/tokyo-night` 实时预览深色和浅色主题；Automatic 会根据当前 IDE 终端背景即时选择对应配色，并保存下次启动使用的自动组合。你还可以调整外框、雨景、配额、图标和状态模块，并查看本机 Token 趋势和模型分布。
...
| 状态   | 模型、思考、路径、Git 分支、提供商限额、Codex 限额、Kimi 限额、Tokens、费用、上下文 |
| 用量   | 最近 3/7/30 天或全部对话的本机 Token 趋势、Top 5 模型曲线与可筛选图例                    |
...
Usage 模块按需读取本机 Pi 会话 JSONL 文件，不会发起网络请求。趋势和模型分布仅使用可可靠归属到 assistant 消息的 `input + output + cacheWrite`；不计入缓存读取及无法归属模型的 tool/summary 用量。它支持最近 3/7/30 天及全部对话范围，仅显示 Token 用量最高的 5 个模型，以及必要时的 Other 系列，不显示 Total 曲线。已解析的 Token 元数据会按会话文件大小和修改时间缓存，绝不缓存 prompt 或对话内容。使用 `←/→` 切换时间范围，`↑/↓` 选择图例，`Enter`/`Space` 显示或隐藏对应系列；Y 轴在当前时间范围内保持固定，只隐藏被取消的曲线。X 轴空间足够时显示起点/中点/终点日期，否则显示起点/终点。
